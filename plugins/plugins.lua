...
local function list_all_plugins(only_enabled, msg)
  ...
  text = '<code>'..text..'</code>\n\n'..nsum..' <b>📂الملفات المثبته</b>\n\n'..nact..' <i>✔️الملفات المفعله</i>\n\n'..nsum-nact..' <i>❌الملفات المعطله</i>'..tmp
...
local Clang = redis:get(Chash)
  ...
  if is_sudo(msg) then
  if (matches[1]:lower() == 'plist' and not Clang) or (matches[1]:lower() == 'الملفات' and Clang) then --after changed to moderator mode, set only sudo
    return list_all_plugins(false, msg)
  end
end
  -- Re-enable a plugin for this chat
  if (matches[1]:lower() == 'pl' and not Clang) or (matches[1]:lower() == 'ملف' and Clang) then
  if matches[2] == '+' and ((matches[4] == 'chat' and not Clang) or (matches[4] == 'كروب' and not Clang)) then
      if is_mod(msg) then
  ...
  if (matches[1]:lower() == 'reload' and not Clang) or (matches[1]:lower() == 'تحديث' and Clang) and is_sudo(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true, msg)
  end
end
