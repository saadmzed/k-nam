...
tdcli_function ({
  ...
  if not lang then
    ...
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "_تم ترقية جميع ادمنية المجموعه الى ادمنيه البوت 🇮🇶_", 0, "md")
     end
 end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "*لا يوجد مستخدمين محضورين في المجموعه 🇮🇶*"
              end
				end
       if not lang then
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
        if not lang then
					...
					return "*لا يوجد مكتومين في المجموعه 🇮🇶*"
             end
				end
      if not lang then
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['whitelist']) == nil then --fix way
     if not lang then
					...
					return "*القائمه البيضاء خاليه 🇮🇶*"
              end
				end
       if not lang then
   message = '*Users of white list :*\n'
         else
   message = '_القائمه البيضاء vip 🇮🇶 :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "*لا يوجد محضورين عام حاليا 🇮🇶*"
             end
				end
        if not lang then
  ...
  if not data[tostring(msg.chat_id_)]['filterlist'] then
    ...
    save_data(_config.moderation.data, data)
      ...
      if not lang then
       ...
       filterlist = '_قائمة الكلمات الممنوعه 🇮🇶 :_\n'
    end
 local i = 1
   for k,v in pairs(data[tostring(msg.chat_id_)]['filterlist']) do
